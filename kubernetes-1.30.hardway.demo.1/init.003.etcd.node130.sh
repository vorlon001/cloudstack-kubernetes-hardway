...
cfssl gencert \
  ...
  -hostname=node13${i}.cloud.local,node13${i},192.168.200.${i},127.0.0.1,localhost \
  -profile=kubernetes \
...
scp /root/IMAGES/etcd-v3.5.13-linux-amd64.tar.gz root@192.168.200.${i}:/root
#pdsh -R ssh -w root@192.168.200.${i} "wget -q --show-progress --https-only --timestamping 'https://github.com/etcd-io/etcd/releases/download/v3.5.13/etcd-v3.5.13-linux-amd64.tar.gz'"
pdsh -R ssh -w root@192.168.200.${i} "tar -xvf /root/etcd-v3.5.13-linux-amd64.tar.gz"
pdsh -R ssh -w root@192.168.200.${i} "sudo mv /root/etcd-v3.5.13-linux-amd64/etcd* /usr/local/bin/"
